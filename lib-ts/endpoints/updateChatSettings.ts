...
import { chatSettingEntry, chatSettingsKeys, updateChatSettingsResponse } from "../types/endpoints/chatSettings";
...
export async function updateChatSettings(sym: string, broadcaster_id: string | undefined, settings: chatSettingEntry, customtoken?: string) {
    return new Promise<updateChatSettingsResponse>(async (resolve, reject) => {
        ...
        Object.keys(settings).forEach(setting => {
            ...
            // @ts-ignore
            if (reqbody[setting] || !chatSettingsKeys.includes(setting) || (Object.values(ignoredIfFalsy).includes(setting) && (settings[Object.keys(ignoredIfFalsy)[Object.values(ignoredIfFalsy).indexOf(setting)]] === false))) return;
        ...
        request(`${urls._url("twitch", "updatechatsettings")}?broadcaster_id=${broadcaster_id_}&moderator_id=${moderator_id}`, { method: urls.twitch.updatechatsettings.method, headers: urls.twitch._headers(sym, customtoken, clientid), body: JSON.stringify(reqbody) }, (e, r) => {
