...
export async function unban(sym: string, broadcaster_id: string | undefined, user_id: string, customtoken?: string) {
    return new Promise<void>(async (resolve, reject) => {
        if ((!(sym ?? undefined) && !(customtoken ?? undefined))) return reject(Error(`sym and customtoken are undefined`));
        if (!(broadcaster_id ?? undefined) || !(user_id ?? undefined)) return reject(Error(`broadcaster_id and/or target_user_id is undefined`));
        ...
        if (!i.regex.numregex().test(user_id_) && i.regex.twitch.usernamereg().test(user_id_)) {
        ...
        request(`${urls._url("twitch", "unban")}?broadcaster_id=${broadcaster_id_}&moderator_id=${moderator_id}&user_id=${user_id_}`, { method: urls.twitch.unban.method, headers: urls.twitch._headers(sym, customtoken, clientid) }, (e, r) => {
