...
export async function ban(sym: string, broadcaster_id: string | undefined, target_user_id: string, duration?: string, reason?: string, customtoken?: string) {
    return new Promise<banResponse>(async (resolve, reject) => {
        ...
        if (!(target_user_id ?? undefined)) return reject(Error(`target_user_id is undefined`));
        ...
        request(`${urls._url("twitch", "bans")}?broadcaster_id=${broadcaster_id_}&moderator_id=${moderator_id}`, { method: urls.twitch.bans.method, headers: urls.twitch._headers(sym, customtoken, clientid), body: JSON.stringify(reqbody) }, (e, r) => {
