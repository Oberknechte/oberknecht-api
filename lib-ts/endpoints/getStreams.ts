...
export async function getStreams(sym: string, filters?: getStreamsFiltersType, customtoken?: string) {
    return new Promise<getStreamsResponse>(async (resolve, reject) => {
        ...
        Object.keys(filters_)?.forEach(filter => {
            reqqueryparams += joinUrlQuery(filter, filters_[filter], (reqqueryparams.length === 0 ? true : false));
        });
