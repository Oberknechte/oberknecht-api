...
async function deleteMessage(sym, broadcaster_id, message_id, customtoken) {
    return new Promise(async (resolve, reject) => {
        if (!(broadcaster_id ?? undefined) || !(message_id ?? undefined)) return reject(Error(`broadcaster_id and/or message_id is undefined`));
        ...
        request(`${urls._url("twitch", "deletemessage")}?broadcaster_id=${broadcaster_id}&moderator_id=${moderator_id}&message_id=${message_id}`, { method: urls.twitch.deletemessage.method, headers: urls.twitch._headers(sym, customtoken, clientid) }, (e, r) => {
