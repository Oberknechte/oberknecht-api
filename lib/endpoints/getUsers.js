...
async function getUsers(sym, logins, ids, customtoken) {
    return new Promise(async (resolve, reject) => {
        ...
        if (!(ids ?? undefined) && (idsinlogins.length > 0)) {
            ids = idsinlogins; idsinlogins.forEach(a => logins_.splice(logins_.indexOf(a)));
        };

        logins_ = ((logins_ ?? undefined) && !Array.isArray(logins_) ? [logins_] : logins_ ?? []).map(a => cleanChannelName(a));
        ...
        let chunks = _chunkArray([...logins_, ...ids], (i.apiclientData[sym]._options.use3rdparty?.getUsers ? 50 : 100));
        let ret = {
            logins: {},
            ids: {},
            details: {}
