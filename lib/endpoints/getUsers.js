        ...
        await Promise.all(
            chunks.map(chunk => {
                ...
                return new Promise((resolve2, reject2) => {
                    let url = `${urls._url("twitch", "users")}${_joinurlquery("login", chunkLogins, true)}${_joinurlquery("id", chunkIDs, ((logins ?? []).length == 0 ? true : false))}`;
                    if (i.apiclientData[sym]._options?.use3rdparty?.getUsers) url = `${urls._url("ivrfitwitch", "users")}${(chunkLogins.length > 0 ? `?login=${chunkLogins}` : "")}${(chunkIDs.length > 0 ? `${(chunkLogins.length > 0 ? "&" : "?")}ids=${chunkIDs}` : "")}`
