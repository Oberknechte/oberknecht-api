...
async function _getUsers(sym, logins, ids, customtoken) {
    return new Promise(async (resolve, reject) => {
        ...
        logins = convertToArray(logins)?.map(a => cleanChannelName(a));
        ids = convertToArray(ids)?.map(a => a?.toLowerCase());

        let idsinlogins = [...(convertToArray(logins ?? [])?.filter(a => regex.numregex().test(a)) ?? [])];
