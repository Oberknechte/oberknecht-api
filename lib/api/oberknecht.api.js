...
class oberknechtAPI {
    ...
    constructor(options) {
        ...
        if (options.saveIDs) {
            ...
            let userssplitter = data.jsonsplitters.users = new jsonsplitter({
                debug: options.debug,
                startpath: options.startPath,
                max_keys_in_file: 20000
            });
            if (Object.keys(userssplitter._mainpaths).length === 0) {
                this.userssplitterpromise = userssplitter.create({ "logins": {}, "ids": {} })
                    .then(a => {
    ...
    async destroy() {
        return new Promise((resolve, reject) => {
            // if (i.apiclientData[this.symbol].filechange_interval) clearInterval(i.apiclientData[this.symbol].filechange_interval);
